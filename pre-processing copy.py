...
dataDir = './data/'  # check_dir('/Volumes/1/data/')
...
for i in dataFiles:
    if os.path.splitext(i)[-1] == defaultZipFileExtension:  # all zip files
        ...
        if not os.path.isdir(dataDir + os.path.splitext(i)[0]):
...
#get integrated PROMPTS file
if not file_exist(integratedPROMPTSFilePath):
    open(integratedPROMPTSFilePath, 'a')

dataPROMPTSPathList = [dataDir + check_dir(i) + promptsSubDir + 'PROMPTS' for i in targetDataFolder]
...
command_line = 'HERest  -A -D -T 1 -C ./manual/config -I ./manual/wintri.mlf -t 250.0 150.0 3000.0 -s stats -S ./manual/train.scp -H ./manual/hmm11/macros -H ./manual/hmm11/hmmdefs -M ./manual/hmm12 ./manual/triphones1'
cmd_list.append(command_line)
cmd(command_line)
